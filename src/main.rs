...
fn main() {
    ...
    loop {
        ...
        let white_extra_game_state = game_state.clone();
        if !game_state.play_turn(&white_extra_game_state, player_1) {
            println!("Black won!");
            return;
        }

        let black_extra_game_state = game_state.clone();
        println!("{}", game_state.format());
        if !game_state.play_turn(&black_extra_game_state, player_2) {
            println!("White won!");
            return;
        }
    }
...
fn get_players<'a>() -> HashMap<
        String,
        Box<Fn(&'a GameState, &Vec<Move<'a>>) -> Option<Move<'a>>>> {
